...
async function main() {
  // Buscamos si ya existe el usuario, y si no, lo creamos automáticamente
  let alumno = await prisma.usuario.findFirst();

  if (!alumno) {
    console.log("⚠️ No se encontró ningún usuario. Creando usuario por defecto...");
    ...
    console.log(`✨ Usuario creado con éxito: ${alumno.nombre_completo}`);
  }
  ...
  for (const item of historial) {
    ...
    console.log(`✅ ${item.nom} procesada y guardada.`);
  }

  console.log("🎉 ¡Listo! Base de datos actualizada con los nuevos cambios.");
}
